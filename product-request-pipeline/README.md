...
1. **Claude.ai** with these connectors enabled:
   - **Notion** — for Product Requests and Domain Specs databases
   - **Linear** — for creating engineering tickets
...
4. **Slack** channel for product request notifications (e.g. `#product-requests`)
...
### Configuration
Update these values in `SKILL.md` for your workspace:
```
...
Linear Team: NG
Slack Channel: #product-requests (configurable on first run)
```
