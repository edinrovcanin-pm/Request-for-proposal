...
- **Slack Channel:** `#legacy-product-requests` (channel ID: `C0ALPV74EH5`)
...
Full pipeline scan:
1. Query the correct Notion Product Requests database for entries with Status = "New" **OR Status is blank/empty** (form submissions may not set Status automatically)
2. **For entries with blank Status:** Immediately set `Status` → "New" and `date:Date Submitted:start` → today's date (ISO-8601) if Date Submitted is also missing. This normalizes form submissions.
3. **Send Slack notification** to the pipeline's channel for each new request found (see Slack Notifications section)
4. For each request, run **Quality Review** (see below)
5. Present findings to user with approve/decline/revise options (use clickable buttons when possible)
6. Wait for user decision before proceeding
7. After processing all requests, run **Status Sync**

> **Why step 2?** Notion forms do not always populate default property values like Status or Date Submitted. The pipeline must handle this gracefully by normalizing records before processing.

### "Generate domain spec for [title]"
1. Fetch the specific request from Notion
2. **Check the `Additional Document` property** on the request page. If a file or URL is attached:
   - Download/fetch its contents (PDF via `read_file`, Google Doc/URL via `google_drive_fetch` or `web_fetch`)
   - Treat the document contents as **primary source material** alongside the request fields — it often contains business requirements, stakeholder decisions, legacy system docs, or domain context that must be reflected in the spec
   - If the document can't be read (unsupported format, access denied), inform the user and ask them to paste the relevant content
3. Generate domain spec using the template in `references/domain-spec-template.md`, incorporating both the request fields and the Additional Document contents
4. Create spec page in Domain Specs database
5. Link spec to the original request via the Domain Spec relation
6. Update request status to "Spec Generated"

### "Generate PRD for [title]" (NextGen only)
1. Fetch the domain spec from Notion
2. **Check the `Additional Document` property** on the original Product Request page. If a file or URL is attached and wasn't already incorporated in the domain spec, fetch and incorporate its contents as additional context for the PRD (same approach as domain spec generation above)
3. Generate PRD using the template in `references/prd-template.md`
...
4. **Attach Additional Document to Epic (if it exists):**
   - Check the original Product Request's `Additional Document` property
   - If a file/URL is present, use Linear's `create_attachment` tool to attach it to the Epic ticket
   - Use the original filename as the attachment title (e.g., "Requirements - Feature X.pdf")
   - This ensures engineers have direct access to supplementary documentation from the Epic without digging through Notion
5. **Update ALL Notion fields on the Product Request:**
...
- [📎/—] Additional Document: [attached / none]
...
Key rules:
- **Always check `Additional Document` first** — if the Product Request has a file/URL in this property, fetch and read it before writing the spec. This document often contains the richest source of requirements, business context, and stakeholder decisions. Weave its content throughout the spec rather than treating it as an appendix.
...
- **If `Additional Document` exists on the Product Request**, also incorporate its contents — it may contain stakeholder context, business rules, or legacy system details that enrich the PRD beyond what the domain spec captured
...
| Legacy | #legacy-product-requests | `C0ALPV74EH5` |
...
Both Notion databases have a `Slack Notified` (checkbox) property. This tracks whether a Slack notification has been sent for a request.

- **When sending Event 1 (New Request):** After sending the Slack notification, set `Slack Notified` → `__YES__` on the Notion page
- **Cron job:** A standalone Node.js cron job (`product-request-notifier`) runs every 5 minutes on a private server, checking both databases for entries where `Slack Notified` is unchecked. It sends the Slack notification and marks them as notified. This handles form submissions automatically 24/7.
- **During "check the base":** If a request has `Slack Notified` unchecked, send the notification and mark it. If already checked, skip (the cron job already handled it).
- **@mention ALL requestors:** If multiple people are in the Requestor field, @mention all of them (comma-separated), not just the first one.

### Rules

- **Always @mention ALL requestors** using `<@SLACK_USER_ID>` format — resolve each requestor's Slack ID separately
- Resolve Slack user ID from requestor's email (Notion person field → email → `slack_search_users`)
- Send notification **once per event** — check `Slack Notified` checkbox to avoid duplicates
