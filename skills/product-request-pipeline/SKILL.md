...
- **Linear Team:** `NG` (NextGen Development — same team, tickets go to Triage)
...
3. Create all tickets in Linear **NG team** with status "Triage" (both pipelines use NG team)
...
Domain Spec → PRD → Linear Tickets (all generated automatically in sequence)

**Legacy or Bug Fix flow (option 1):**
Linear Tickets directly (no spec or PRD needed)
...
Slack is used **only to notify about new requests**. Nothing else — no ticket creation notifications, no status syncs, no spec generation alerts.

### Channels

| Pipeline | Channel | Channel ID |
|----------|---------|------------|
| NextGen | #product-requests | (configurable) |
| Legacy | #legacy-product-requests | `C0ALWRULM0E` |

### When to notify

**Only when new requests are found during "check the base" / "check new requests" / "check legacy" / "check nextgen".**

For each request with Status = "New", send one message to the pipeline's Slack channel:
...
- Send notification **once per new request** when detected during scan
- Do NOT notify for: ticket creation, spec generation, status changes, declines, or revisions
